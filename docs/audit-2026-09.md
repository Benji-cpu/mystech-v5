...
| 9 | **Deck generation asks for a picturable object** | **The last art failure: 20 prompts, 0 abstractions, 0 exclusions** |
| 10 | The daily card email invites instead of going silent | A user who turns it on hears something |
| 11 | Input validation on 53 of 71 mutating routes | Nothing visible; every prompt-bound field now has a ceiling |

## 6. Still open

Everything filed on the day is now closed. MysTech has zero open CRM rows.

| What | Who | Where |
|---|---|---|
| ~~Stability AI balance is zero~~ | — | Topped up 2026-09-16, 978 credits |
| ~~`VERCEL_TOKEN` dead~~ | — | Rotated across all four projects that held it |
| ~~Abstract card prompts still draw a figure~~ | — | Fixed at deck-generation time; 20 prompts, 0 abstractions |
| ~~51 mutating API routes with no input validation~~ | — | 53 of 71 validated, up from 20 |
| ~~The Daily Card email skips every user without a deck~~ | — | It sends the invitation instead, once |
| Google Cloud TTS billing off — read-aloud 403s | **Ben** | CRM #174 — the same switch as WordZoo |
| Six `needs_review` rows on the reading and Chronicle flows | **Ben** | CRM #54, #55, #63–#66 |
| 28 React Compiler lint errors | Claude, someday | Not filed — see below |
| Planned but never built: person cards (12), true deck collaboration (16) | Product decision | `docs/ROADMAP.md` |

The lint errors are deliberately not a CRM row. They are all one React Compiler rule
("Calling setState synchronously within an effect"), none is a live bug, and fixing them is a
refactor of most of the interactive components. A row for them would sit on the board being
true and never actionable, which is the failure mode this audit was written to stop.
