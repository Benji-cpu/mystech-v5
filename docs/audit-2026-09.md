...
1. **The Stability AI balance is zero.** Every card generation returns
   `402 payment_required`. There is no fallback image. *(Still true — only Ben can fix it.)*
...
horoscope filler. But the card is a placeholder glyph, because there is no art. **The
product's central artefact is a grey rectangle.**
...
**Verdict.** The walk has no dead ends now. It still ends on a deck with no pictures, and
that is a billing problem, not a code problem.
...
Mucha's entire body of work is women. v2 strips "oracle card", "tarot card" and
"in the style of &lt;artist&gt;" from the style prompt, keeping the palette and the
ornament. It is written, in the harness, and has never run: it needs Stability credits.
...
| — | **Card art prompt** | **Measured, not shipped — needs credits to finish** |

## 6. Still open

| What | Who | Where |
|---|---|---|
| Stability AI balance is zero — no new deck gets any art | **Ben** | CRM, blocking |
| `VERCEL_TOKEN` dead — deployment ingest 403s nightly | **Ben** | CRM |
| Google Cloud TTS billing off — read-aloud 403s | **Ben** | CRM |
| Finish the art prompt work (run v2, decide) | Claude, once credits exist | CRM |
