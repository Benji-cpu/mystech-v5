"use client";

import { useReducer, useRef, useEffect, useState, useMemo, useCallback } from "react";
import gsap from "gsap";
import { Playfair_Display } from "next/font/google";
import {
  AURORA,
  PHASE_FORMATION,
  PHASE_MOOD,
  FORMATIONS,
  type PhaseId,
} from "./aurora-journey-theme";
import { journeyReducer, initialState } from "./aurora-journey-state";
import { AuroraConstellation } from "./aurora-constellation";
import { AuroraConnections } from "./aurora-connections";
import { AuroraRibbons, type AuroraRibbonHandle } from "./aurora-ribbons";
import { AuroraGoldenThread } from "./aurora-golden-thread";

import { AwakeningPhase } from "./phases/awakening-phase";
import { GatheringPhase } from "./phases/gathering-phase";
import { CreationPhase } from "./phases/creation-phase";
import { RevelationPhase } from "./phases/revelation-phase";
import { ReturnPhase } from "./phases/return-phase";

const playfair = Playfair_Display({ subsets: ["latin"], display: "swap" });

// ── Phase label display ─────────────────────────────────────────────

const PHASE_LABELS: Record<PhaseId, string> = {
  awakening: "Awakening",
  gathering: "The Gathering",
  creation: "Creation",
  revelation: "Revelation",
  return: "Return",
};

// ── Shell component ─────────────────────────────────────────────────

export function AuroraJourneyShell() {
  const [state, dispatch] = useReducer(journeyReducer, initialState);
  const auroraRef = useRef<AuroraRibbonHandle>(null);
  const shellRef = useRef<HTMLDivElement>(null);
  const phaseIndicatorRef = useRef<HTMLParagraphElement>(null);
  const constellationSpacerRef = useRef<HTMLDivElement>(null);
  const breathOverlayRef = useRef<HTMLDivElement>(null);
  const [containerSize, setContainerSize] = useState({ width: 0, height: 0 });

  // Track container size for golden thread pixel → viewBox conversion
  useEffect(() => {
    const el = shellRef.current;
    if (!el) return;

    const update = () => {
      setContainerSize({ width: el.offsetWidth, height: el.offsetHeight });
    };
    update();

    const ro = new ResizeObserver(update);
    ro.observe(el);
    return () => ro.disconnect();
  }, []);

  // Execute aurora commands when state changes
  useEffect(() => {
    if (state.auroraCommand.type !== "idle" && auroraRef.current) {
      auroraRef.current.executeCommand(state.auroraCommand);
    }
  }, [state.auroraCommand]);

  // Current mood for aurora ribbons
  const currentMood = PHASE_MOOD[state.phase];

  // Golden thread targets
  const goldenThreads = useMemo(() => {
    const vega = FORMATIONS[state.formation]?.vega;
    if (!vega) return [];

    if (
      state.subPhase === "name_input" ||
      state.subPhase === "intention_input" ||
      state.subPhase === "revealing"
    ) {
      return [
        {
          id: `thread-${state.subPhase}`,
          fromCx: vega.cx,
          fromCy: vega.cy,
          toX: containerSize.width / 2,
          toY: containerSize.height * 0.65,
        },
      ];
    }
    return [];
  }, [state.formation, state.subPhase, containerSize]);

  // Speaking state for constellation
  const isSpeaking =
    state.subPhase === "greeting" ||
    state.subPhase === "name_absorb" ||
    state.subPhase === "intention_absorb" ||
    state.subPhase === "interpreting";

  // GSAP-driven phase indicator animation
  useEffect(() => {
    const el = phaseIndicatorRef.current;
    if (!el) return;

    gsap.fromTo(
      el,
      { opacity: 0, y: -8 },
      { opacity: state.isBreathPause ? 0.2 : 0.5, y: 0, duration: 0.4, ease: "power2.out" }
    );
  }, [state.phase, state.isBreathPause]);

  // GSAP-driven constellation spacer height
  useEffect(() => {
    const el = constellationSpacerRef.current;
    if (!el) return;

    const isInterpretingOrComplete =
      state.phase === "revelation" &&
      (state.subPhase === "interpreting" || state.subPhase === "complete");

    gsap.to(el, {
      height: isInterpretingOrComplete ? "15%" : "30%",
      duration: 0.6,
      ease: "power2.inOut",
    });
  }, [state.phase, state.subPhase]);

  // GSAP-driven breath overlay
  useEffect(() => {
    const el = breathOverlayRef.current;
    if (!el) return;

    gsap.to(el, {
      opacity: state.isBreathPause ? 1 : 0,
      duration: 0.3,
    });
  }, [state.isBreathPause]);

  return (
    <div
      ref={shellRef}
      className={`h-[100dvh] w-full flex flex-col overflow-hidden relative ${playfair.className}`}
      style={{ background: AURORA.bg }}
    >
      {/* Layer 0: Deep background gradient */}
      <div
        className="absolute inset-0 z-0"
        style={{
          background: `radial-gradient(ellipse at 50% 30%, rgba(15, 10, 40, 0.6), ${AURORA.bg} 70%)`,
        }}
      />

      {/* Layer 1: Aurora ribbons (Canvas 2D) */}
      <AuroraRibbons ref={auroraRef} mood={currentMood} />

      {/* Layer 2: SVG constellation (React Spring) */}
      <div className="absolute inset-0 z-20 pointer-events-none" style={{ height: "40%" }}>
        <AuroraConstellation
          formation={state.formation}
          showConnections={state.showConnections}
          userStars={state.userStars}
          isBreathPause={state.isBreathPause}
          isSpeaking={isSpeaking}
        />
        <AuroraConnections
          formation={state.formation}
          showConnections={state.showConnections}
          isBreathPause={state.isBreathPause}
        />
      </div>

      {/* Layer 4: Golden thread overlay */}
      <AuroraGoldenThread
        threads={goldenThreads}
        containerWidth={containerSize.width}
        containerHeight={containerSize.height}
      />

      {/* Layer 3: DOM content zones */}
      <div className="relative z-30 flex flex-col flex-1 min-h-0">
        {/* Phase indicator */}
        <div className="shrink-0 pt-3 pb-1 px-4 text-center">
          <p
            ref={phaseIndicatorRef}
            className="text-[10px] sm:text-xs uppercase tracking-[0.3em]"
            style={{ color: AURORA.accentDim, opacity: 0 }}
          >
            {PHASE_LABELS[state.phase]}
          </p>
        </div>

        {/* Constellation spacer */}
        <div
          ref={constellationSpacerRef}
          className="shrink-0"
          style={{ height: "30%" }}
        />

        {/* Content zone */}
        <div className="flex-1 min-h-0 flex flex-col relative">
          {/* Breath pause overlay */}
          <div
            ref={breathOverlayRef}
            className="absolute inset-0 z-50 pointer-events-none"
            style={{
              background: `radial-gradient(ellipse at center, transparent 30%, ${AURORA.bg}80 100%)`,
              opacity: 0,
            }}
          />

          {/* All phases always mounted */}
          <PhaseWrapper isActive={state.phase === "awakening"}>
            <AwakeningPhase
              subPhase={state.subPhase as never}
              dispatch={dispatch}
              auroraRef={auroraRef}
              isActive={state.phase === "awakening"}
            />
          </PhaseWrapper>

          <PhaseWrapper isActive={state.phase === "gathering"}>
            <GatheringPhase
              subPhase={state.subPhase as never}
              dispatch={dispatch}
              auroraRef={auroraRef}
              isActive={state.phase === "gathering"}
              userName={state.userName}
              userIntention={state.userIntention}
            />
          </PhaseWrapper>

          <PhaseWrapper isActive={state.phase === "creation"}>
            <CreationPhase
              subPhase={state.subPhase as never}
              dispatch={dispatch}
              auroraRef={auroraRef}
              isActive={state.phase === "creation"}
              selectedStyleId={state.selectedStyleId}
              userName={state.userName}
            />
          </PhaseWrapper>

          <PhaseWrapper isActive={state.phase === "revelation"}>
            <RevelationPhase
              subPhase={state.subPhase as never}
              dispatch={dispatch}
              auroraRef={auroraRef}
              isActive={state.phase === "revelation"}
              revealedCards={state.revealedCards}
              interpretationProgress={state.interpretationProgress}
              userName={state.userName}
            />
          </PhaseWrapper>

          <PhaseWrapper isActive={state.phase === "return"}>
            <ReturnPhase
              subPhase={state.subPhase as never}
              dispatch={dispatch}
              auroraRef={auroraRef}
              isActive={state.phase === "return"}
              userName={state.userName}
              loopCount={state.loopCount}
            />
          </PhaseWrapper>
        </div>

        {/* Safe area bottom */}
        <div className="shrink-0 pb-[env(safe-area-inset-bottom)]" />
      </div>

      {/* Vignette overlay */}
      <div
        className="absolute inset-0 pointer-events-none z-40"
        style={{
          background:
            "radial-gradient(ellipse at center, transparent 50%, rgba(10, 8, 32, 0.4) 100%)",
        }}
      />
    </div>
  );
}

// ── Phase wrapper ───────────────────────────────────────────────────
// GSAP-driven visibility instead of Framer Motion

function PhaseWrapper({
  isActive,
  children,
}: {
  isActive: boolean;
  children: React.ReactNode;
}) {
  const ref = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const el = ref.current;
    if (!el) return;

    gsap.to(el, {
      opacity: isActive ? 1 : 0,
      duration: 0.3,
      onComplete: () => {
        if (el) {
          el.style.visibility = isActive ? "visible" : "hidden";
          el.style.pointerEvents = isActive ? "auto" : "none";
        }
      },
    });

    // Set immediately for initial state
    if (isActive) {
      el.style.visibility = "visible";
    }
  }, [isActive]);

  return (
    <div
      ref={ref}
      className="absolute inset-0 flex flex-col"
      style={{
        opacity: 0,
        visibility: "hidden",
        pointerEvents: "none",
      }}
    >
      {children}
    </div>
  );
}
