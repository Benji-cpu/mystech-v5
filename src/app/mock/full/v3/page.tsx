...
import { lunarReducer, lunarInitialState } from "./lunar-state";
import { LunarParticles } from "./lunar-particles";
import { lunar } from "./lunar-theme";

import { LunarDashboard } from "./views/dashboard";
import { LunarMyDecks } from "./views/my-decks";
import { LunarCreateDeck } from "./views/create-deck";
import { LunarDeckView } from "./views/deck-view";
import { LunarCardDetail } from "./views/card-detail";
import { LunarArtStyles } from "./views/art-styles";
import { LunarReadingFlow } from "./views/reading-flow";
import { LunarSettings } from "./views/settings";
...
export default function LunarTideMock() {
  const [state, dispatch] = useReducer(lunarReducer, lunarInitialState);
  ...
  function renderView() {
    switch (state.currentView) {
      case "dashboard":
        return <LunarDashboard onNavigate={navigate} />;
      case "decks":
        return <LunarMyDecks onNavigate={navigate} />;
      case "create-deck":
        return <LunarCreateDeck onNavigate={navigate} onBack={goBack} />;
      case "deck-detail":
        return (
          <LunarDeckView
      ...
      case "art-styles":
      case "art-style-detail":
        return <LunarArtStyles onNavigate={navigate} />;
      case "reading":
        return (
          <LunarReadingFlow
            onNavigate={navigate}
      ...
      case "settings":
        return <LunarSettings onNavigate={navigate} />;
      default:
        return <LunarDashboard onNavigate={navigate} />;
    }
  }
  ...
  return (
    ...
    >
      ...
      <LunarParticles mood={state.mood} />
        ...
        <AnimatePresence>
          {!state.hideNav && (
            <motion.nav
              ...
              style={{
                background: `${lunar.bg}ee`,
                borderColor: `${lunar.border}4d`,
              }}
            >
              {/* Crescent moon logo */}
              <div
                ...
                style={{
                  ...
                  border: `1px solid ${lunar.glow}30`,
                }}
              >
                <svg
                  className="w-5 h-5"
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      color: isActive ? lunar.glow : lunar.muted,
                    }}
                  >
                    {isActive && (
                      <motion.div
                        layoutId="lunar-desktop-nav-active"
                    ...
                    <span className="relative z-10 text-[9px] mt-0.5 font-medium">
                      {item.label}
                    </span>
                  </button>
                );
          ...
          <AnimatePresence>
            {canGoBack && !state.hideNav && (
              ...
              >
                <button
                  onClick={goBack}
                  className="flex items-center gap-1.5 text-sm transition-colors min-h-[44px]"
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <motion.div
                        layoutId="lunar-mobile-nav-active"
                        className="absolute inset-0 rounded-xl"
                        style={{
                          background: `${lunar.glow}15`,
                          border: `1px solid ${lunar.glow}30`,
