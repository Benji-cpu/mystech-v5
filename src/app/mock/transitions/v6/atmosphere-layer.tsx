...
import type { AtmosphereHandle } from "./types";

interface AtmosphereLayerProps {
  className?: string;
}

export const AtmosphereLayer = forwardRef<AtmosphereHandle, AtmosphereLayerProps>(
  function AtmosphereLayer({ className }, ref) {
    ...
    useImperativeHandle(ref, () => ({
      splat: (x, y, dx, dy, r, g, b, radius) => {
        simRef.current?.splat(x, y, dx, dy, r, g, b, radius);
      },
    }));
...
function initFluidSim(canvas: HTMLCanvasElement, isMobile: boolean) {
  ...
  function doSplat(
    ...
    const splatRadius = (radius ?? SPLAT_RADIUS) / 100;
    ...
    setUniform1f(splatProg, "radius", splatRadius);
    blit(splatProg);
    velocity.swap();
