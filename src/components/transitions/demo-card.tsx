...
export function DemoCard({
  ...
  return (
    <div
      className={cn(
        sizes[size],
        "relative rounded-xl overflow-hidden border border-gold/40 shadow-lg shadow-purple-900/20",
        "bg-gradient-to-b from-surface-mid to-surface-deep",
        className
      )}
      style={style}
    >
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-3">
        <p className="text-center text-xs font-medium text-gold tracking-wide">
          {title}
        </p>
      </div>
      {/* Star accent */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 text-gold/30 text-2xl pointer-events-none">
        ✦
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="w-16 h-16 border border-gold/30 rotate-45" />
        <div className="absolute w-12 h-12 border border-gold/20 rounded-full" />
        <div className="absolute w-20 h-20 border border-gold/10 rounded-full" />
      </div>
      <div className="absolute inset-[3px] rounded-[9px] border border-gold/20 pointer-events-none" />
    </div>
  );
