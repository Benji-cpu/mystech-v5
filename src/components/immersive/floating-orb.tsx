...
export function FloatingOrb() {
  ...
  return (
    <>
      {/* Radial nav items */}
      <AnimatePresence>
        {isOrbExpanded && !focusMode && <RadialNav />}
      </AnimatePresence>

      {/* The orb itself — stays mounted, animates opacity/scale for focus mode */}
      <motion.button
        onClick={toggleOrb}
        ...
        animate={{
          ...
          scale: focusMode ? 0.5 : 1,
          width: orbSize,
          height: orbSize,
          rotate: isOrbExpanded ? 45 : 0,
        }}
        whileHover={focusMode ? {} : { scale: 1.1 }}
        whileTap={focusMode ? {} : { scale: 0.95 }}
      ...
      >
        ...
        {/* Pulsing glow ring — hidden when orb is expanded or in focus mode */}
        {!isOrbExpanded && !focusMode && (
          <motion.div
