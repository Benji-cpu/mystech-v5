...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Base color: either from texture or uniform
    vec3 base = uBaseColor;
    if (uHasTexture > 0.5) {
      base = texture2D(uCardTexture, vUv).rgb;
    }

    // Holographic blend
    float holoBlend = uIntensity * (0.6 + shimmer * 0.4);
    vec3 holoColor;
    if (uHasTexture > 0.5) {
      // Texture mode: blend rainbow on top at overlay opacity
      holoColor = mix(base, rainbowColor, uOverlayOpacity * holoBlend);
    } else {
      // Pure holographic mode (original behavior)
      holoColor = mix(base, rainbowColor, holoBlend);
    }
...
export function HolographicMaterial({
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uCardTexture: { value: texture || new THREE.Texture() },
  ...
  useFrame(({ clock }) => {
    ...
    if (texture) {
      materialRef.current.uniforms.uCardTexture.value = texture;
