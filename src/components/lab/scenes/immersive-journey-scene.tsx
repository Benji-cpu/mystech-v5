...
function OriginIntroContent() {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", width: HTML_WIDTH, height: HTML_HEIGHT, gap: 10 }}>
...
function OriginSelectionContent({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: HTML_WIDTH, height: HTML_HEIGHT, gap: 4, paddingTop: 12, overflow: "hidden" }}>
      <div style={{ color: "rgba(201,169,78,0.8)", fontSize: 12, fontWeight: 500, letterSpacing: "0.04em", textAlign: "center", marginBottom: 2 }}>
        How many cards?
      </div>

      <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 3 }}>
        {CARD_COUNT_OPTIONS.map((opt, i) => {
      ...
      {/* Description area */}
      <div style={{ minHeight: 44, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, padding: "0 6px" }}>
        {activeOption && (
...
function OriginInputContent({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: HTML_WIDTH, height: HTML_HEIGHT, gap: 8, paddingTop: 14, overflow: "hidden" }}>
...
function OriginCard({
  ...
  visible: boolean;
  phase: JourneyPhase;
  pendingPhase: JourneyPhase | null;
  forgingSubPhase: ForgingSubPhase | null;
  ...
  const htmlWrapperBackRef = useRef<HTMLDivElement>(null);
  ...
  // Trigger flip when pendingPhase changes (intro→selection, selection→input)
  useEffect(() => {
    if (pendingPhase && pendingPhase !== displayPhase) {
      // Record start angle for progress-based midpoint
      flipStart.current = flipAngle.current;
      flipTarget.current = flipTarget.current === 0 ? Math.PI : 0;
      flipInProgress.current = true;
      midpointFired.current = false;
    }
  }, [pendingPhase, displayPhase]);

  useFrame((_, delta) => {
    if (!groupRef.current || !cardGroupRef.current) return;
    ...
    if (visible && !isForging) {
      ...
      // Revealing/completion — hide origin card, JourneyCards take over
      targetScale = 0;
      lerpRate = delta * 4;
    ...
    // Apply opacity and pointer-events via refs (no React state in useFrame)
    const opacityStr = String(htmlOpacityRef.current);
    const pointerVal = htmlOpacityRef.current > 0.1 ? "auto" : "none";
    if (htmlWrapperRef.current) {
      htmlWrapperRef.current.style.opacity = opacityStr;
      htmlWrapperRef.current.style.pointerEvents = pointerVal;
    }
    if (htmlWrapperBackRef.current) {
      htmlWrapperBackRef.current.style.opacity = opacityStr;
      htmlWrapperBackRef.current.style.pointerEvents = pointerVal;
    }

    // Flip animation
    if (flipInProgress.current) {
      flipAngle.current = THREE.MathUtils.lerp(flipAngle.current, flipTarget.current, delta * 5);

      // Progress-based midpoint detection (fire at 40% of total flip)
      const totalFlip = Math.abs(flipTarget.current - flipStart.current);
      const progress = totalFlip > 0.01
        ? Math.abs(flipAngle.current - flipStart.current) / totalFlip
        : 0;

      if (!midpointFired.current && progress > 0.4) {
      ...
      if (Math.abs(flipAngle.current - flipTarget.current) < 0.02) {
        flipAngle.current = flipTarget.current;
        flipInProgress.current = false;
      }
    }

    cardGroupRef.current.rotation.y = flipAngle.current;
  });
    ...
    <group ref={groupRef} scale={0}>
      <Float speed={2} rotationIntensity={isForging ? 0 : 0.3} floatIntensity={isForging ? 0 : 0.5} floatingRange={[-0.2, 0.2]}>
        <group ref={cardGroupRef}>
          ...
          {/* Front Html — always mounted, hidden via backface-visibility */}
          <Html
            position={[0, 0, 0.05]}
            center
            transform
          ...
          {/* Back Html — always mounted, hidden via backface-visibility */}
          <Html
            position={[0, 0, -0.05]}
            center
            transform
            ...
            style={{ backfaceVisibility: "hidden", pointerEvents: "none" }}
          >
            <div ref={htmlWrapperBackRef} style={{ ...htmlContainerStyle, opacity: 1, pointerEvents: "auto" }}>
              {displayPhase === "intro" && <OriginIntroContent />}
              {displayPhase === "selection" && (
                <OriginSelectionContent onSelect={onCardCountSelect} />
              )}
    ...
    <div className="flex flex-col items-center gap-2 pointer-events-auto">
      ...
      {isLastPage && (
        <div className="flex flex-wrap gap-3 justify-center mt-2">
          <button
            onClick={onReset}
            className="rounded-lg border border-border bg-secondary/50 px-5 py-2 text-sm font-medium text-secondary-foreground transition-colors hover:bg-secondary/80"
          >
            Start Over
          </button>
          <button
...
export default function ImmersiveJourneyScene() {
  const [phase, setPhase] = useState<JourneyPhase>("intro");
  const [pendingPhase, setPendingPhase] = useState<JourneyPhase | null>(null);
  ...
  // Auto-advance from intro → selection (via flip)
  useEffect(() => {
    if (phase === "intro" && autoAdvanceIntro) {
      const timer = setTimeout(() => {
        setPendingPhase("selection");
      }, introDelay * 1000);
      return () => clearTimeout(timer);
    }
  }, [phase, autoAdvanceIntro, introDelay]);

  // Called at the midpoint of a flip — commit the phase change
  ...
  const handleCardCountSelect = useCallback((count: number) => {
    setCardCount(count);
    // Trigger flip to input phase
    setPendingPhase("input");
  }, []);

  const handleInputSubmit = useCallback((_text: string) => {
    // No flip for input → forging; card shrinks to 0 instead
    setPhase("forging");
  }, []);
    ...
    <div className="relative h-full w-full">
      <LabCanvas camera={{ position: [0, 0, 6], fov: 50 }}>
        <JourneyContent
          ...
          onFlipMidpoint={handleFlipMidpoint}
        />
      </LabCanvas>
      ...
      {/* Top bar: Reset + page size toggle */}
      {phase !== "intro" && (
